...
def create_full_image_decoder(sR, eR, generator_outputs_channels, noise = True):
    ...
    with tf.variable_scope("decoder_fc1"):
        fc1_output = gen_fc(initial_input, output_channels=4096)
    ...
    z = tf.reshape(fc2_bn, [-1, 4, 4, config.ngfI*8])

    # Add noise only at train time
    if config.mode == "train":
        layer_specs = [
            (config.ngf * 8, 0.5, 4, (2,2)),   # decoder_6: [batch, 4, 4, ngf * 8] => [batch, 8, 8, ngf * 8]
            (config.ngf * 8, 0.5, 4, (2,2)),   # decoder_5: [batch, 8, 8, ngf * 8] => [batch, 16, 16, ngf * 8]
            (config.ngf * 4, 0.5, 4, (2,2)),   # decoder_4: [batch, 16, 16, ngf * 8] => [batch, 32, 32, ngf * 4]
            (config.ngf * 2, 0.0, 4, (2,2)),   # decoder_3: [batch, 32, 32, ngf * 4] => [batch, 64, 64, ngf * 2]
            (config.ngf, 0.0, 4, (2,2)),       # decoder_2: [batch, 64, 64, ngf * 2] => [batch, 128, 128, ngf]
        ]
    else:
        layer_specs = [
            (config.ngf * 8, 0.0, 4, (2,2)),   # decoder_6: [batch, 4, 4, ngf * 8] => [batch, 8, 8, ngf * 8]
            (config.ngf * 8, 0.0, 4, (2,2)),   # decoder_5: [batch, 8, 8, ngf * 8] => [batch, 16, 16, ngf * 8]
            (config.ngf * 4, 0.0, 4, (2,2)),   # decoder_4: [batch, 16, 16, ngf * 8] => [batch, 32, 32, ngf * 4]
            (config.ngf * 2, 0.0, 4, (2,2)),   # decoder_3: [batch, 32, 32, ngf * 4] => [batch, 64, 64, ngf * 2]
            (config.ngf, 0.0, 4, (2,2)),       # decoder_2: [batch, 64, 64, ngf * 2] => [batch, 128, 128, ngf]
        ]
